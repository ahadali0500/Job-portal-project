...
import { compareDates, containsNumber, getCurrentDate, getCurrentTimestamp, humanReadableToISO, isoToHumanReadable, splitDateRange, validateEmail } from "@/app/utils/helper.api";
import { useSession } from "next-auth/react";
import { useRouter } from "next/navigation";

export default function ControlledDemo({ params }) {
  ...
  const [Image, setImage] = useState({ "profileImage": '', "backgroundImage": '' });
  ...
  const handlesubmit = async (e) => {
    ...
    if (id == "") {
      ...
      try {
        ...
        Data.append('created_At', getCurrentTimestamp());
        Data.append('image', profileImage);
        Data.append('backgroundImage', backgroundImage);
        Data.append('background_image', Image.backgroundImage);
        Data.append('profile_image', Image.profileImage);

        const response = await axios.post(`${API_URL}/update_cv.php`, Data);
        if (response.data.code == 200) {
          toast.current.show({ severity: 'success', detail: response.data.message, life: 6000 });
          window.open(`https://desired-techs.com/job-bank/resume/${folder}/index.php?id=${params.id}`, '_blank')
  ...
  const loadMoreData = async () => {
    try {
      ...
      const response = await axios.post(`${API_URL}/fetch_ind_cv.php`, formData);
      if (response.data) {
        console.log(response.data);
        const dt = JSON.parse(response.data.experienceData)
        const updatedExperienceData = dt.map(item => {
          const [startDate, endDate] = splitDateRange(item.experiencePeriod);
          return {
            ...item,
            StartDate: humanReadableToISO(startDate),
            EndDate: endDate !== "Present" ? humanReadableToISO(endDate) : null,
            chkdata: endDate === "Present"
          };
        });
        ...
        setcontactData(data => ({
          ...
          current_position: response.data.currentPosition,
        ...
        setEducationData(JSON.parse(response.data.education))
        setWorkExperienceData(updatedExperienceData)
        setskillData(JSON.parse(response.data.skills))
        setlanguageData(JSON.parse(response.data.languages))
        setid(response.data.cv_temp)
        setfolder(response.data.cv_temp)
        setImage(data => ({
  ...
  return (
    <>
      <Toast ref={toast} />
      <Breadcrumbs name="Update Resume" />
